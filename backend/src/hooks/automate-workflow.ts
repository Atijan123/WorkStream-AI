 ...
 */
export class AutomateWorkflowHook extends BaseHook {
  name = 'automate_workflow';
  description = 'Processes natural language workflow descriptions and generates workflow specifications';
  ...
  private async parseWorkflowRequest(request: string): Promise<Omit<Workflow, 'id'>> {
  ...
  private cleanWorkflowName(name: string): string {
    return name
      .replace(/[^\w\s-]/g, '')
      .trim()
      .split(' ')
  ...
  private extractTrigger(request: string): Workflow['trigger'] {
  ...
  private extractActions(request: string): WorkflowAction[] {
  ...
  private generateDescription(request: string, trigger: Workflow['trigger'], actions: WorkflowAction[]): string {
  ...
  private describeCronSchedule(schedule: string): string {
    ...
    if (schedule.includes('* * 1')) return 'weekly on Monday';
    
    return `on schedule: ${schedule}`;
  }

  private describeAction(action: WorkflowAction): string {
    switch (action.type) {
      case 'fetch_data':
